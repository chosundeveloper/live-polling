...
### Keyword Canvas 디스플레이

- `display-mindmap.html` 파일은 concept.png 느낌의 **화이트 배경 워드 클라우드 뷰**를 제공합니다.
- 문장에서 추출된 단어만을 퍼센트 좌표에 잔잔히 배치하고, 빈도에 따라 크기·광도를 조절합니다.
- `?question=2`처럼 쿼리 파라미터로 질문을 지정할 수 있고, `?mock=1` 옵션이나 상단 `MOCK` 버튼으로 샘플 데이터를 켜서 연출을 확인할 수 있습니다.
- `SHUFFLE` 버튼을 누르면 동일한 단어 집합을 새로운 위치로 재배열해 화면 구성을 빠르게 바꿀 수 있습니다.
